...
void print_gpk(const GroupPublicKey &gpk) {
    ...
    auto print_gt = [](const std::string &label, const gt_t g_const) {
        gt_t g;
        gt_new(g);
        gt_copy(g, g_const);  // copy because g_const is const array
        int len = gt_size_bin(g, 0);
        std::vector<uint8_t> buf(len);
...
void serialize_signature(const Signature &sig, std::vector<uint8_t> &out) {
    // helper to serialize an EC point with length prefix
    auto append_ep = [&](const ep_t &e) {
        int len = ep_size_bin(e, 1);
        uint32_t nlen = htonl(static_cast<uint32_t>(len));
        // write length
        out.insert(out.end(),
                   reinterpret_cast<uint8_t*>(&nlen),
                   reinterpret_cast<uint8_t*>(&nlen) + sizeof(nlen));
        // write point bytes
        std::vector<uint8_t> buf(len);
        ep_write_bin(buf.data(), len, e, 1);
        out.insert(out.end(), buf.begin(), buf.end());
    ...
    // helper to serialize a big-number with length prefix
    auto append_bn = [&](const bn_t &b) {
        int len = bn_size_bin(b);
        uint32_t nlen = htonl(static_cast<uint32_t>(len));
        out.insert(out.end(),
    ...
    append_bn(sig.c);
    append_bn(sig.s_alpha);
    append_bn(sig.s_beta);
    append_bn(sig.s_x);
    append_bn(sig.s_delta1);
    append_bn(sig.s_delta2);
    append_bn(sig.s_y);
}
...
int main() {
    core_init();                   // REQUIRED: initializes RELIC
    ep_param_set_any_pairf();      // REQUIRED: sets up pairing parameters
    bn_t order; bn_new(order);     
    ep_curve_get_ord(order);
    //conf_print();  // disables internal RELIC printouts
    GroupPublicKey gpk; GroupManagerSecretKey gmsk;
    //bn_t order; bn_new(order);
    BBS_Setup(gpk, gmsk);
    //print_gpk(gpk);
    GroupMemberKey gsk = BBS_Join(gpk, gmsk, order);
    //bn_t bpg_order; bn_new(bpg_order); ep_curve_get_ord(bpg_order);

    int server_fd = socket(AF_INET, SOCK_STREAM, 0);
    sockaddr_in addr{};
    addr.sin_family = AF_INET; addr.sin_port = htons(9002);
    addr.sin_addr.s_addr = INADDR_ANY;
    bind(server_fd, (sockaddr*)&addr, sizeof(addr)); listen(server_fd, 1);
    std::cout << "📡 Waiting for receiver...\n";
    int client_sock = accept(server_fd, nullptr, nullptr);
    std::cout << "✅ Receiver connected\n";

    std::vector<uint8_t> gpk_buf;
    serialize_gpk(gpk, gpk_buf);
    int gpk_len = gpk_buf.size();
    send(client_sock, &gpk_len, sizeof(int), 0);
    send(client_sock, gpk_buf.data(), gpk_buf.size(), 0);
    //std::cout << "📦 Sent gpk of length: " << gpk_buf.size() << std::endl;

    char ack[64] = {0};
    recv(client_sock, ack, sizeof(ack), 0);
    std::cout << "📨 Client says: " << ack << "\n";

    // DSS key and Idv_i creation
    bn_t sk_i; bn_new(sk_i); bn_rand_mod(sk_i, order);
    ep_t pk_i; ep_new(pk_i); ep_mul(pk_i, gpk.g1, sk_i);

    std::string D_i = "user1234";
    std::vector<uint8_t> D_bytes(D_i.begin(), D_i.end());
    int pk_len = ep_size_bin(pk_i, 1);
    std::vector<uint8_t> pk_buf(pk_len);
    ep_write_bin(pk_buf.data(), pk_len, pk_i, 1);

    std::map<std::string, std::vector<uint8_t>> idv_map;
    idv_map["D_i"] = D_bytes;
    idv_map["pk_i"] = pk_buf;
    std::vector<uint8_t> Idv_buf;
    serialize_map(idv_map, Idv_buf);

    Signature sigma_g = BBS_Sign(gpk, gsk, std::string(Idv_buf.begin(), Idv_buf.end()), order);
    std::vector<uint8_t> sig_buf;
    serialize_signature(sigma_g, sig_buf);

    std::map<std::string, std::vector<uint8_t>> cert_map;
    cert_map["Idv_i"] = Idv_buf;
    cert_map["sigma_g"] = sig_buf;
    std::vector<uint8_t> cert_buf;
    serialize_map(cert_map, cert_buf);

    // Timestamp signing (DSS)
    std::string ts = "2025-04-29T18:20:00Z";  // or use current timestamp dynamically
    std::vector<uint8_t> ts_bytes(ts.begin(), ts.end());

    bn_t sigma_t; bn_new(sigma_t);
    DSS_Sign(sk_i, ts, sigma_t);

    int siglen = bn_size_bin(sigma_t);
    std::vector<uint8_t> sigma_t_buf(siglen);
    bn_write_bin(sigma_t_buf.data(), siglen, sigma_t);

    // Final auth request map
    std::map<std::string, std::vector<uint8_t>> auth_request;
    auth_request["cert_g"] = cert_buf;
    auth_request["ts"] = ts_bytes;
    auth_request["sigma_t"] = sigma_t_buf;

    std::vector<uint8_t> auth_buf;
    serialize_map(auth_request, auth_buf);

    int auth_len = auth_buf.size();
    send(client_sock, &auth_len, sizeof(int), 0);
    send(client_sock, auth_buf.data(), auth_len, 0);

    std::cout << "📤 Authentication request sent\n";
    auto start = std::chrono::high_resolution_clock::now();

    int resp_len = 0;
    recv(client_sock, &resp_len, sizeof(int), 0);

    std::vector<char> resp_buf(resp_len + 1, 0);  // +1 for null-terminator
    recv(client_sock, resp_buf.data(), resp_len, 0);

    auto end = std::chrono::high_resolution_clock::now();
    std::cout << "📬 Server response: " << resp_buf.data() << std::endl;

    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);

    std::cout << "Authentication Execution Time: " << duration.count() << " microseconds\n";

    close(client_sock); close(server_fd); core_clean(); return 0;
}